...
&emsp;&emsp;从名字很容易可以看出来，`RuntimeSpecialConfig` 的生命周期要更长一些。`SpecialConfig` 的生命周期为构建配置到配置完成导入，而 `RuntimeSpecialConfig` 的生命周期会从构建配置持续到 swpp 结束运行。
...
3. 除 `IndivisibleConfig` 外，所有的 `RuntimeSpecialConfig` 都会携带与 `IndivisibleConfig` 相同的副作用（因为 swpp 无法预测对象可能包含的内容）。
...
&emsp;&emsp;注：延迟初始化配置可以读取其它延迟初始化配置，但注意不能形成循环依赖，如果出现循环依赖 swpp 将会抛出异常。
