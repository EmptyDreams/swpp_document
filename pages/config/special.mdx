...
&emsp;&emsp;从名字很容易可以看出来，`RuntimeSpecialConfig` 的声明周期要更长一些，实际上也正是如此，`SpecialConfig` 的生命周期为构建配置到配置完成导入，而 `RuntimeSpecialConfig` 的声明周期会从构建配置持续到 swpp 结束运行。
...
defineConfig({
    runtimeDep: {
      // 注意这些工具函数可以也只能在配置项名称后方使用，且不可嵌套使用
